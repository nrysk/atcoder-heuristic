...
class Solver:
    ...
    def solve(self):
        """
        焼きなまし法で巡回経路を最適化する
        理論上、全てのマスを1周して後、2周するまでに全てのマスを0にすることができる
        マスが正の値の場合は全て積む
        ...
        best_route = [i for i in range(self.n * self.n)] + [
            i for i in range(self.n * self.n)
        ]
        ...
        while True:
            ...
            offset = random.choice([0, self.n * self.n])
            if random.random() < 0.2:
                i, j = random.sample(range(self.n * self.n), 2)
                new_route[offset + i], new_route[offset + j] = (
                    new_route[offset + j],
                    new_route[offset + i],
                )
            elif random.random() < 0.5:
                i, j = sorted(random.sample(range(self.n * self.n), 2))
                new_route[offset + i : offset + j + 1] = reversed(
                    new_route[offset + i : offset + j + 1]
                )
            else:
                i, j = sorted(random.sample(range(self.n * self.n), 2))
                new_route = (
                    new_route[: offset + i]
                    + new_route[offset + i : offset + j + 1][::-1]
                    + new_route[offset + j + 1 :]
                )
