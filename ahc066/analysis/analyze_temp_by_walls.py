...
label_files = glob.glob("out_label_temp/*.csv")
if not label_files:
    print(
        "❌ out_label_temp ディレクトリ内にCSVファイルが見つかりません。_label.py を先に実行してください。"
    )
    exit()
...
df["normalized_walls"] = (df["num_h_walls"] * df["num_v_walls"]) // (df["n"] * 10)
...
for wall_val in wall_groups:
    ...
    plt.xlabel("Max Temperature", fontsize=10)
