...
if not label_files:
    print(
        "❌ No CSV files found in 'out_label' directory. Please run the labeling script first."
    )
    exit()
...
# 保存用のフォルダを作成
OUTPUT_FOLDER = "plots_turn_by_walls"
os.makedirs(OUTPUT_FOLDER, exist_ok=True)

# 規格化された壁の数のユニークな値を抽出してソート
wall_groups = sorted(df["normalized_walls"].unique())

print(f"=== Starting num_turn analysis by wall density groups ===")
...
for wall_val in wall_groups:
    ...
    # 2. ⚡ 各ラベルグループ（0, 1, 2）の内側で、num_turn の割合(%)を計算
    # これにより、件数の格差に潰されず「上位5%がどの回転数を好むか」が1対1で目視できます
    counts = sub_df.groupby(["label", "num_turn"]).size().reset_index(name="count")
    ...
    # 4. 見た目の装飾・見やすさの調整
    plt.title(
        f"Best num_turn Analysis (for num_walls // n = {wall_val})",
        fontsize=13,
        fontweight="bold",
    )
    plt.xlabel("num_turn (マクロ内での方向転換数)", fontsize=10)
    ...
    # 5. 画像として保存
    output_path = os.path.join(OUTPUT_FOLDER, f"turn_analysis_walls_{wall_val}.png")
    plt.tight_layout()
...
print(
    f"\n=== 🎉 All plots generated successfully! Check the '{OUTPUT_FOLDER}' folder ==="
)
