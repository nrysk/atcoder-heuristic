...
public class ClimbSimulator : Simulator
{
    ...
    public int Evaluate(string firstOperations, string macro)
    {
        ...
        for (int step = 0; step < Steps; step++)
        {
            ...
            int newScore = EvaluateRoute(firstOperations, macro, _newRoute);
...
public class AnnealSimulator : Simulator
{
    ...
    private static readonly string[] FirstOpsCandidates = { "", "F", "L", "R", "RR" };
    ...
    public List<char> CalcBestOperations(string firstOperations, string macro)
    {
        ...
        // --- 初動(firstOperations)の状態変数 ---
        string currentFirstOps = firstOperations;
        string bestFirstOps = firstOperations;
        ...
        for (int step = 0; step < Steps; step++)
        {
            ...
            // --- 今回のステップで試す初動 ---
            string newFirstOps = currentFirstOps;

            double mode = Rand.NextDouble();

            // 確率で、経路はいじらずに初動（firstOperations）だけをランダムに変更する
            if (mode < 0.05)
            {
                newFirstOps = FirstOpsCandidates[Rand.Next(FirstOpsCandidates.Length)];
            }
            else if (mode < 0.2) // swap 
            {
                (_newRoute[i], _newRoute[j]) = (_newRoute[j], _newRoute[i]);
            }
            else if (mode < 0.6) // reverse 
            ...
            // --- 新しい経路と新しい初動の組み合わせで評価 ---
            int newScore = EvaluateRoute(newFirstOps, macro, _newRoute);
            ...
            if (delta < 0 || Math.Exp(-delta / temperature) > Rand.NextDouble())
            {
                ...
                currentFirstOps = newFirstOps; // 初動の状態も更新

                for (int k = 0; k < M; k++) currentRoute[k] = _newRoute[k];

                if (currentScore < bestScore)
                {
                    Console.Error.WriteLine($"New best score: {currentScore} at step {step} (firstOperations: {currentFirstOps}, macro: {macro})");
                    bestScore = currentScore;
                    bestFirstOps = currentFirstOps; // ベストな初動を記憶
        ...
        // --- 最終的に一番良かった初動と経路を渡す ---
        return BuildOperations(bestFirstOps, macro, bestRoute);
    }
