...
class DataGenerator:
    ...
    def generate(self) -> list[str]:
        ...
        # マクロをランダムに生成して評価する。重複を避けるためにセットで管理する
        macro_set = set()
        while len(macro_set) < 500:
            ...
            for macro in macros:
                if macro not in macro_set:
                    macro_set.add(macro)
                    score = simulator.evaluate(macro, max_temperature=max_temperature)
                    print(
                        f'{self.n},{self.m},{self.num_walls},"{macro}",{score},{num_forward},{num_turn},{ends_look_back}',
                        file=sys.stdout,
                    )
