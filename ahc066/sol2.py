...
def solve(
    ...
    # マクロを登録する
    operations.extend(["M"] + ["F"] * macro_forward_count + ["M"])
    current_y, current_x = distance_map.forward(
        0, 0, 3, macro_forward_count
    )  # マクロ前進の移動先を計算して保存する

    # 現在の位置と向きを保存する
    current_idx = current_y * n + current_x
...
def main():
    ...
    FIRST_MACRO_FORWARD_COUNT = 3
    distance_map = LazyDistanceMap(
        n, h, v, macro_forward_count=FIRST_MACRO_FORWARD_COUNT
    )
    best_operations = solve(
        ...
        ball_idxs,
        basket_idxs,
        macro_forward_count=FIRST_MACRO_FORWARD_COUNT,
    )
    for macro_forward_count in range(FIRST_MACRO_FORWARD_COUNT + 1, min(n - 2, 15) + 1):
        ...
        if len(operations) < len(best_operations):
            print(
                f"Updated best operations: Macro forward count: {macro_forward_count}, operations: {len(operations)}",
                file=sys.stderr,
                end="",
            )
