...
def arrange_conveyors(
    ...
    """
    搬出口を通る横向きのベルトコンベアを1つ配置する。
    横幅2マスの縦向きのベルトコンベアをN/2個配置する。
    全て時計周りを順方向とする。
    """
    ...
    # 横向きのベルトコンベア
    conveyor = []
    conveyor.extend([(0, i) for i in range(n)])
    conveyor.extend([(1, i) for i in reversed(range(n))])
    conveyors.append(conveyor)

    # 縦向きのベルトコンベア
    for i in range(0, n, 2):
        conveyor = []
        conveyor.extend([(j, i) for j in reversed(range(n))])
        conveyor.extend([(j, i + 1) for j in range(n)])
        conveyors.append(conveyor)
...
def solve(
    ...
    while min_number < n * n:
        ...
        conveyor_idx = min_number_position[1] // 2 + 1
        direction = 1 if min_number_position[1] % 2 == 0 else -1
        ...
        conveyor_idx = 0
        direction = 1 if min_number_position[1] < n // 2 else -1
