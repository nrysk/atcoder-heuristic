...
def solve(
    ...
    while min_number < n * n:
        ...
        while True:
            if shuffle_probability > 0 and random.random() < shuffle_probability:
                # シャッフル用のベルトコンベアをランダムに回転させる
                conveyor_idx = len(conveyors) - 1
...
def main():
    ...
    while True:
        ...
        result = solve(n, a, shuffle_probability=0.005)
        if len(result[1]) < len(best_result[1]):
            print(
                f"New best result: {len(result[1])} operations (previous best: {len(best_result[1])} operations)",
                flush=True,
                file=sys.stderr,
            )
            best_result = result
